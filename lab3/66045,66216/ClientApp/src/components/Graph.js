...
import { LinearGrdient } from '@vx/gradient'
...
function Node({node}) {
  ...
  return(
    <Group top={node.y} left={node.x}>
      {node.depth !== 0 && (
        <circle r={12} fill={bg} stroke={isParent ? white : circus}
        onClick={ () => { alert(`clicked: ${JSON.stringify(node.data.name)}`);
      }}
      />
      )}
...
function RootNode({node}) {
  ...
  return(
    <Group top={node.y} left={node.x}>
      ...
      <text dy={'.33em'} fontSize={9} fontFamily="Arial" textAnchor={'middle'} style={{pointerEvents: 'none'}} fill={bg}
      >
        {node.data.name}
      </text>
    </Group>
